...
const gdpPerCapita = ref(null);
const gniPerCapita = ref(null);
...
async function fetchEconomicData() {
  try {
    ...
    gdpPerCapita.value = gdpResponse.data[1][0]?.value || 'N/A';
    gniPerCapita.value = gniResponse.data[1][0]?.value || 'N/A';
  } catch (error) {
    console.error('Error fetching economic data:', error);
  }
}
